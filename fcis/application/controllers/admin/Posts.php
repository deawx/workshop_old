...
class Posts extends Admin_Controller {
	...
	public function index()
	{
		...
		// 	'total_rows' => count($this->Post->find())
		// );
		$config	= array(
			'base_url' => site_url(uri_string()),
			'total_rows' => count($this->Post->find_active()),
			'per_page' => 1
		);
		...
		{
			$q = $this->input->get('q');
			$this->data['posts'] = $this->Post->find($config['per_page'], $this->input->get('offset'),$user_id, $q);
		...
		{
			$this->data['posts'] = $this->Post->find($config['per_page'], $this->input->get('offset'),$user_id);
		}
