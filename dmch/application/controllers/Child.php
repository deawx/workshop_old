...
class Child extends Public_Controller {
  ...
  function index()
  {
    $tables = $this->common->search('child')->result_array();
    $row = array();
    foreach ($tables as $_k => $_t) :
      $report = anchor('child/report/'.$_t['id'],'<i class="fa fa-file-pdf-o fa-lg"></i>',['class'=>'btn btn-link']);
      $row[$_k]['no']	= $_k+1;
      $row[$_k]['birthdate']	= $_t['birthdate'];
      $row[$_k]['fullname']	= anchor('child/view/'.$_t['id'],$_t['name'].nbs(3).$_t['surname']);
      $row[$_k]['gender']	= $_t['gender'].'<span class="pull-right">'.$report.'</span>';
    endforeach;
  ...
  function report($id)
  {
    ...
    $pdf->AddPage();
    $this->params['child'] = $this->common->search_id('child',$id)->row();
    $this->params['child_no'] = $this->common->search_id('mother_pregnant',$this->params['child']->mother_id)->row();
    $this->params['child_all'] = $this->db->where('mother_id',$this->params['child']->mother_id)->count_all_results('mother_pregnant');
    $this->params['mother'] = $this->common->search_id('mother',$this->params['child']->mother_id)->row();
    $this->params['staff'] = $this->common->search_id('staff',$this->params['child']->staff_id)->row();
    $this->params['organize'] = $this->common->search_id('organize',$this->params['child']->organize_id)->row();
    $html = $this->load->view('_pages/report', $this->params, TRUE);
    $pdf->writeHTMLCell(0, 0, '', '', $html, 0, 1, 0, true, '', true);
    $pdf->Output('tet.pdf', 'I');
    die();
  }

  function view($id)
  {
    if ( ! (int)$id) redirect('child');
    ...
    $card = isset($profile['card']) ? $profile['card'] : '';
    ...
    $religion = isset($profile['religion']) ? $profile['religion'] : '';
    $anomaly = isset($profile['anomaly']) ? $profile['anomaly'] : '';
    $health = isset($profile['health']) ? $profile['health'] : '';
    ...
    $this->params['form'] = array(
      ...
      array('label'=>form_label(ucfirst('บัตรประชาชน'),'card',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_input(array('name'=>'card','class'=>'form-control','required'=>TRUE,'min'=>'0','max'=>'9999999999999'),set_value('card',$card)),
          'help'=>'กรอกข้อมูล ตัวเลขบัตรประชาชนไม่เกิน 13 หลัก'),
      ...
      array('label'=>form_label(ucfirst('ส่วนสูง'),'height',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_number(array('name'=>'height','class'=>'form-control','required'=>TRUE,'min'=>'0','max'=>'99'),set_value('height',$height)),
          'help'=>'กรอกข้อมูล ตัวเลขส่วนสูง'),
      array('label'=>form_label(ucfirst('น้ำหนัก(กรัม)'),'weight',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_number(array('name'=>'weight','class'=>'form-control','required'=>TRUE,'min'=>'0','max'=>'99999'),set_value('weight',$weight)),
          'help'=>'กรอกข้อมูล ตัวเลขน้ำหนัก'),
      array('label'=>form_label(ucfirst('ขนาดศีรษะ'),'round',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_number(array('name'=>'round','class'=>'form-control','required'=>TRUE,'min'=>'0','max'=>'99'),set_value('round',$round)),
          'help'=>'กรอกข้อมูล ตัวเลขเส้นรอบศีรษะ'),
      array('label'=>form_label(ucfirst('ศาสนา'),'religion',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_input(array('name'=>'religion','class'=>'form-control','required'=>TRUE),set_value('religion',$religion)),
          'help'=>''),
      array('label'=>form_label(ucfirst('ความผิดปกติแต่กำเนิด'),'anomaly',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_textarea(array('name'=>'anomaly','class'=>'form-control','required'=>TRUE),set_value('anomaly',$anomaly)),
          'help'=>''),
      array('label'=>form_label(ucfirst('สุขภาพร่างกาย'),'health',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_textarea(array('name'=>'health','class'=>'form-control','required'=>TRUE),set_value('health',$health)),
          'help'=>'')
    );
  ...
  function physical($id,$pid='')
  {
    if ( ! (int)$id) redirect('child');
    $data = $this->input->post();
    if ($data) :
      $this->common->save('child_physical',$data);
      $this->callback('บันทึกข้อมูล พัฒนาการทารก');
      redirect($this->agent->referrer());
    endif;
    $profile = $this->common->search_id('child',$id)->row_array();
    $physical = $this->common->search_id('child_physical',$pid)->row_array();
    $weight = isset($physical['weight']) ? $physical['weight'] : '';
    $height = isset($physical['height']) ? $physical['height'] : '';
    $round = isset($physical['round']) ? $physical['round'] : '';
    $month = isset($physical['month']) ? $physical['month'] : $this->db->where('child_id',$id)->from('child_physical')->count_all_results()+1;
    $this->params['head'] = 'ข้อมูล พัฒนาการทางด้านร่างกาย';
    $hidden = ((int)$pid) ? ['id'=>$pid] : ['child_id'=>$id,'month'=>$month];
    $this->params['hidden'] = $hidden;
    $this->params['form'] = array(
      array('label'=>form_label(ucfirst('เดือนที่'),'month',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_number(array('name'=>'month','class'=>'form-control','required'=>TRUE,'disabled'=>TRUE,'min'=>'0','max'=>'99'),set_value('month',$month)),
          'help'=>''),
      array('label'=>form_label(ucfirst('ส่วนสูง'),'height',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_number(array('name'=>'height','class'=>'form-control','required'=>TRUE,'min'=>'0','max'=>'99'),set_value('height',$height)),
          'help'=>'กรอกข้อมูล ตัวเลขส่วนสูง'),
      array('label'=>form_label(ucfirst('น้ำหนัก(กก.)'),'weight',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_number(array('name'=>'weight','class'=>'form-control','required'=>TRUE,'min'=>'0','max'=>'99'),set_value('weight',$weight)),
          'help'=>'กรอกข้อมูล ตัวเลขน้ำหนัก'),
      array('label'=>form_label(ucfirst('ขนาดศีรษะ'),'round',array('class'=>'control-label text-right col-sm-3')),
          'input'=>form_number(array('name'=>'round','class'=>'form-control','required'=>TRUE,'min'=>'0','max'=>'99'),set_value('round',$round)),
          'help'=>'กรอกข้อมูล ตัวเลขเส้นรอบศีรษะ')
    ...
    $this->elements['content'] = $this->load->view('_elements/form', $this->params, TRUE);
    $add_physical = anchor('child/physical/'.$id,'<i class="fa fa-plus fa-lg"></i>',array('class'=>'btn btn-link pull-right'));
    $tables = $this->common->search('child_physical',['child_id'=>$id],'60')->result_array();
    $row = array();
    foreach ($tables as $_k => $_t) :
      $edit = anchor('child/physical/'.$id.'/'.$_t['id'],'<i class="fa fa-info fa-lg"></i>',array('class'=>'btn btn-link'));
      $row[$_k]['month']	= $_t['month'];
      $row[$_k]['height']	= $_t['height'];
      $row[$_k]['weight']	= $_t['weight'];
      $row[$_k]['round']	= $_t['round'].'<span class="pull-right">'.$edit.'</span>';
    endforeach;
    $this->table->set_heading(array('เดือนที่','ส่วนสูง','น้ำหนัก','ขนาดศีรษะ'));
    $this->table->set_template(array('table_open'  => '<table class="table table-bordered table-hover datatable">'));
    $this->elements['content'] .= $add_physical.br().'<hr/>'.$this->table->generate($row);
    $this->elements['sidebar'] = $this->_sidebar($profile);
    $this->load->view('_layouts/sidebar', $this->elements);
  }
  ...
  function vaccine($id)
  {
    ...
    $data = $this->input->post();
    if ($data) :
      $this->common->save('child_vaccination',$data);
      $this->callback('บันทึกข้อมูล การฉีดวัคซีน');
      redirect($this->agent->referrer());
    endif;
    $this->params['head'] = 'ข้อมูล การฉีดวัคซีน';
    $vid = $this->common->search('child_vaccination',['child_id'=>$id])->row();
    $this->params['hidden'] = ($vid) ? ['id'=>$vid->id] : ['child_id'=>$id];
    $this->params['vaccine'] = $this->common->search('child_vaccination',array('child_id'=>$id))->row_array();
    ...
    endforeach;
    $this->elements['content'] = $this->load->view('vaccine', $this->params, TRUE);
    $profile = $this->common->search_id('child',$id)->row_array();
    $this->elements['sidebar'] = $this->_sidebar($profile);
    $this->load->view('_layouts/sidebar', $this->elements);
  }
  ...
  function graph($id)
  {
    ...
    $profile = $this->common->search_id('child',$id)->row_array();
    $this->elements['content'] = anchor('child/view/'.$id,'<i class="fa fa-reply fa-lg"></i>',['class'=>'btn btn-primary btn-block']).br();
  ...
  function _sidebar($profile)
  {
    ...
    $this->params['body'] = ([
      anchor('mother/profile/'.$profile['mother_id'],'ข้อมูลมารดา',array('class'=>'btn btn-link')),
      anchor('child/view/'.$profile['id'],'เกี่ยวกับ(ข้อมูลทารก)',array('class'=>'btn btn-link')),
      anchor('child/physical/'.$profile['id'],'พัฒนาการทางด้านร่างกาย',array('class'=>'btn btn-link')),
      anchor('child/vaccine/'.$profile['id'],'การฉีดวัคซีน',array('class'=>'btn btn-link')),
      anchor('child/graph/'.$profile['id'],'กราฟแสดงผล',array('class'=>'btn btn-link','target'=>'_blank'))
    ]);
